...
// La lectura del ticket (o el proveedor) a veces trae una fecha que no existe
// (p. ej. 29 feb en un año no bisiesto) — Postgres la rechaza con "date/time
// field value out of range" y tronaría el guardado completo. Se valida antes
// de escribir nada en vez de dejar que Postgres truene a medio guardado.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const badDateLines = input.lines.filter((l) => l.expiresOn && !isValidCalendarDate(l.expiresOn));
    if (badDateLines.length > 0) {
      const names = badDateLines.map((l) => l.ticketDescription?.trim() || l.description?.trim() || "renglón sin descripción").join(", ");
      return json({ error: `La fecha de caducidad no existe en: ${names}. Corrígela o bórrala antes de guardar.` }, 400);
    }
    ...
    // A partir de aquí ya existe la fila de purchase_receipts: si algo más
    // truena, se borra (junto con las fotos que ya se hayan subido) en vez de
    // dejar un encabezado huérfano sin renglones — Postgrest no da
    // transacciones entre estas llamadas.
